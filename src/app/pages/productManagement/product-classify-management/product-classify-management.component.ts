...
export class ProductClassifyManagementComponent implements OnInit {
	...
	editDeviceOuterPirce() {
		...
		if(!this.deviceSellInfo.outerPrice || this.deviceSellInfo.outerPrice === null || this.deviceSellInfo.outerPrice <= 0) {
		...
		const data = {
			...
			"param":{
				...
				"productRent": this.deviceSellInfo.outerPrice,  // 租金  必传
				"productPrice": this.deviceSellInfo.price,   // 当前价格  必传
